...
- DFS solution:
  ...
  - if we find an 0, begin a dfs on that cell
    ...
    - if the dfs finds a border, none of the Os can be converted as
...
class Solution(object):
    def surroundDFS(self, y, x, visited, board):
        toCapture = set()
        q = [(y, x)]
        ...
        while q:
            ...
            toCapture.add((currY, currX))
            for direction in directions:
                nextY = currY + direction[0]
                nextX = currX + direction[1]
                if board[nextY][nextX] == 'O' and (nextY, nextX) not in visited:
                    q.append((nextY, nextX))
        for coords in toCapture:
            y, x = coords
            board[y][x] = 'X'
        ...
        for y, row in enumerate(board):
            for x, col in enumerate(row):
                if (y, x) not in visited and board[y][x] == "O":
                    self.surroundDFS(y, x, visited, board)
                else:
                    visited.add((y, x))
    ...
    tests = [([["X", "X", "X", "X"],
             ...
             ([["X", "O", "O", "X", "X", "X", "O", "X", "O", "O"],
               ["X", "O", "X", "X", "X", "X", "X", "X", "X", "X"],
                 ...
                 ["O", "X", "X", "X", "X", "X", "O", "X", "X", "X"],
                 ["X", "O", "O", "X", "X", "O", "X", "X", "O", "O"],
                 ["X", "X", "X", "O", "O", "X", "O", "X", "X", "O"]], [["X", "O", "O", "X", "X", "X", "O", "X", "O", "O"],
                                                                       ["X", "O", "X", "X", "X",
                                                                        "X", "X", "X", "X", "X"],
                                                                       ["X", "X", "X", "X", "X",
                                                                        "X", "X", "X", "X", "X"],
