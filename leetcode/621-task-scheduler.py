...
- We can extend this reasoning to arbitrarily many groups of characters. So essentially, if we take our biggest character set, we can "contain" smaller character sets
  in it. We can do this for n many character sets. After this, the space between characters
  is always greater than N, so we can imagine just taking
"""
import collections


class Task:
    def __init__(self, char, count, timestamp):
        self.char = char
        self.count = count
        self.timestamp = timestamp

    def render(self):
        print("char: {0}, count: {1}, ts: {2}".format(
            self.char, self.count, self.timestamp))
...
class Solution:
    def leastInterval(self, tasks, n):
        ...
        taskTracker = {}
        for task in tasks:
            if task not in taskTracker:
                taskTracker[task] = Task(task, 1, -float('inf'))
            else:
                taskTracker[task].count += 1
        taskQueue = sorted([taskTracker[key]
                            for key in taskTracker], key=lambda task: task.count, reverse=True)
        ...
        while (taskQueue):
            ...
            for i in range(len(taskQueue)):
                task = taskQueue[i]
                if task.count and timestamp - task.timestamp > n:
                    taskList += task.char
                    task.count -= 1
