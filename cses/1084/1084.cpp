...
int main() {
  ...
  rep(i, n) {
    lin(applicant);
    applicants.pb(applicant);
  }
  sort(applicants.begin(), applicants.end());

  rep(i, m) {
    lin(apartment);
    apartments.pb(apartment);
  }
  ...
  while (!(i == apartments.size()) && !(j == applicants.size())) {
    ll apartment = apartments[i];
    ll applicant = applicants[j];

    if (((applicant - k) <= apartment) && (apartment <= applicant + k)) {
      pairs++;
      i++;
      j++;
      // output("Match found! (apt, applicant): " << apartment << " " <<
      // applicant);
    } else {
      // output("No match on applicant " << applicant << " and apartment "
      //                                << apartment);
      (apartment < applicant) ? i++ : j++;
    }
  }
  output(pairs);
