...
int main() {
  ...
  map<ll, int> places;
  int l = 0, r = 0, currLen = 1, bestLen = 1;
  places[nums[l]] = l;
  while (r < count) {
    currLen = r - l + 1;
    /*
    if (currLen > bestLen) {
      printf("Updating bestLen to: %d\n", currLen);
    }
    */
    bestLen = (currLen > bestLen) ? currLen : bestLen;

    /*
    printf("Places content: ");
    for (auto it : places) {
    ...
    */
    ...
    r++;
    if ((places.find(nums[r]) != places.end()) && (l <= places[nums[r]])) {
      l = places[nums[r]] + 1;
    }
    places[nums[r]] = r;

    // printf("l: %d (%lld), r: %d (%lld), currLen: %d\n", l, nums[l], r,
    // nums[r], currLen);
  }
  output(bestLen);
