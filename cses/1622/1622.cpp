...
static void solve(const size_t targetLen, string letters, string combination,
                  set<string>& solutions) {
  if (combination.size() == targetLen) {
    if (solutions.find(combination) == solutions.end()) {
  ...
  for (size_t i = 0; i < letters.size(); i++) {
    ...
    letters.erase(i, 1);
    solve(targetLen, letters, combination, solutions);
    combination.erase(combination.size() - 1, 1);
    letters.insert((i == 0) ? 0 : i - 1, letter);
  }
}
...
int main() {
  ...
  string combination = "";
  set<string> solutions;
  solve(inString.size(), inString, combination, solutions);
