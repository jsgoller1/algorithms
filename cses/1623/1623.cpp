...
ll getWeight(const vector<ll>& weights, ll bitstring) {
  ll weight = 0;
  int i = 0;
  // cout << bitset<20>(bitstring) << endl;
  while (bitstring > 0) {
    if (bitstring & 1) {
      weight += weights[i];
    }
...
int main() {
  sanic_io();
  lin(weightsN);
  ll maxInclude = 1 << (weightsN - 1);
  ll totalWeight = 0;
  vector<ll> weights;
  for (ll i = 0; i < weightsN; i++) {
    lin(weight);
    totalWeight += weight;
    weights.push_back(weight);
  }

  ll minDiff = totalWeight;
  ll leftWeight = 0;
  ll rightWeight = 0;
  ll partitionDiff = 0;
  for (ll bitstring = 1; bitstring < maxInclude; bitstring++) {
    leftWeight = getWeight(weights, bitstring);
    rightWeight = totalWeight - leftWeight;
    partitionDiff = (leftWeight > rightWeight) ? leftWeight - rightWeight
                                               : rightWeight - leftWeight;
    minDiff = (partitionDiff < minDiff) ? partitionDiff : minDiff;
  }
  output(minDiff);
