...
ll solve(int n, vector<ll>& solutions) {
  if (n < 0) {
    return 0;
  }
  if (n == 0) {
  ...
  ll total = 0;
  for (ll i = 1; i < 7; i++) {
    // eprintf("Calculating %lld - %lld\n", n, i);
    total += (solve(n - i, solutions) % 1000000007);
  }
  // eprintf("solutions[%lld] = %lld\n", n, total);
  solutions[n] = total;
...
int main() {
  ...
  // read_from_files();
  lin(n);
  std::vector<ll> solutions(n + 1, 0);
  output(solve(n, solutions));
  return 0;
}
