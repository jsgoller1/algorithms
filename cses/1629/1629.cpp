...
bool comp(const pair<ll, ll>& film1, const pair<ll, ll>& film2) {
  if (film1.second == film2.second) {
    return film1.first < film2.first;
  }
  return film1.second < film2.second;
}
...
int main() {
  ...
  rep(i, count) {
    lin(start);
    lin(end);
    movies.pb(pair<ll, ll>(start, end));
  }
  sort(movies.begin(), movies.end(), comp);
  ll films = 0;
  ll currentEnd = 0;
  for (auto film : movies) {
    if (film.first >= currentEnd) {
      films++;
      currentEnd = film.second;
    }
  }
  output(films);
