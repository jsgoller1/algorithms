...
def draw_tree(tree):
    if not tree:
        return "Empty tree."
    output = []
    def recurse(node, spacing=0, child_type=None):
        if not node:
            return 
        output.append(spacing*2*" " + f"{child_type+':' if child_type else ''}{node.data}")
        recurse(node.right, spacing+2, "R")
        recurse(node.left, spacing+2, "L")
    recurse(tree)
    return "\n".join(output)
        
def draw_tree_ascii(node, prefix="", is_left=True):
    """Generate an ASCII visualization of a binary tree."""
    if not node:
        return "Empty tree."
    ...
    def recurse(node, prefix="", is_left=True):
        ...
        result = []
        result.append(f"{prefix}{'|-- L:' if is_left else '|__ R:'}{node.data}")
        if node.left or node.right:
            # If there are children, add branches recursively
            if node.left:
                result += recurse(node.left, prefix + ("|   " if is_left else "    "), True)
            else:
                result.append(f"{prefix}{'|   ' if is_left else '    '}|-- L: None")
            if node.right:
                result += recurse(node.right, prefix + ("|   " if is_left else "    "), False)
            else:
                result.append(f"{prefix}{'|   ' if is_left else '    '}|__ R: None")
        return result

    return "\n".join(recurse(node, prefix, is_left))
