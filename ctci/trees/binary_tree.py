...
class binary_tree:
    ...
    def __insert(self, root, key, val):
        if root is None: # First insertion
            ...
            self.root = bst_node(key, val)
        elif root.key < key:
            if root.right is None:
                # print "Setting key " + str(key) + " to " + str(val)
                root.right = bst_node(key, val)
            else:
                self.__insert(root.right, key, val)
        else:
            if root.left is None:
                # print "Setting key " + str(key) + " to " + str(val)
                root.left = bst_node(key, val)
            else:
...
if __name__ == '__main__':
    ...
    print "Retrieved", bst.retrieve(lookup_val)
    bst.traverse_preorder("3 1 0 2 5 4 6")
    # Postorder:
    bst.traverse_postorder("0 2 1 4 6 5 3")
    bst.traverse_inorder("0 1 2 3 4 5 6")
