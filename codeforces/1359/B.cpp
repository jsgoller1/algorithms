...
static ll solve_line(char* line, ll cost1, ll cost2, ll* cache, ll i) {
  if (i < 0) {
    return 0;
  }
  if (cache[i] == LINF) {
    if (line[i] == '*') {
      cache[i] = solve_line(line, cost1, cost2, cache, i - 1);
    } else {
      ll c1, c2;
      c1 = cost1 + solve_line(line, cost1, cost2, cache, i - 1);
      c2 = (i - 1 >= 0 && line[i - 1] != '*')
               ? cost2 + solve_line(line, cost1, cost2, cache, i - 2)
               : LINF;
      cache[i] = (c1 < c2) ? c1 : c2;
    }
  }
...
static void solve() {
  ...
  while (h--) {
    ...
    total += solve_line(line, cost1, cost2, cache, w - 1);
    // cin >> line[0];  // skips newline
  }
  output(total);
}
