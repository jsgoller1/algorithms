...
static void solve() {
  ...
  ll days = 0;
  int wi = 0, j = 0;
  unordered_set<int> curr_letters;
  int len = word.size();
  while (wi < len) {
    days++;
    j = wi;
    while (curr_letters.size() < 3) {
      curr_letters.insert(word[j++]);
    }
    while (wi < len && curr_letters.find(word[wi]) != curr_letters.end()) {
      wi++;
    }
    curr_letters.clear();
  }
  ...
  while i < len:
    days++
    j = i
    while size of curr_letters < 3:
      insert word[j] in curr_letters
      j++
    while i < len and word[i] in curr_letters:
      i++;
    curr_letters.clear()
  output(days)
  */
