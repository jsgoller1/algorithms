...
void solve(const ll k, vl &differences) {
  sort(differences.begin(), differences.end());
  ll total =0;
  // 11 - 8 = 3; from 0 to 3 is 4 
  rep(i, ll(differences.size())-(k-1)){
    total += differences[i];
  }
  output(total);
  /*
    // can eliminate individual villagers if l = r
    1 3 5 2
    differences:
      2, 2, 3

    1 9 12 4 7 2 (can eliminate 3-1)
     8 3 8 3 5 -> 11

    1 9 8 2 3 3 1 8 7 7 9 2 (can eliminate 8-1)
     8 1 6 1 0 2 7 1 0 2 7
     |   |     | | |   | | 
...
int main() {
  ...
  rep(i, cases) { 
    ...
    rep(j, n-1){
      lin(villager2);
      ll diff = abs(villager1-villager2);
      differences.pb(diff);
      villager1 = villager2;
    }
    solve(k, differences); }
